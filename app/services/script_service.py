...
class ScriptService:
    def list_scripts(self, session: Session, tenant_id: int) -> list[Script]:
        return session.exec(
            select(Script).where(Script.tenant_id == tenant_id).order_by(Script.updated_at.desc()),
        ).all()

    def create_script(self, session: Session, tenant_id: int, name: str, body: str) -> Script:
        script = Script(tenant_id=tenant_id, name=name, body=body, updated_at=datetime.utcnow())
        session.add(script)
        session.commit()
